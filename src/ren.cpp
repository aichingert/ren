...
extern "C" void ren_draw_frame(t_ren* ren) {
    vkWaitForFences(ren->device, 1, &ren->in_flight_fence, VK_TRUE, UINT64_MAX);
    vkResetFences(ren->device, 1, &ren->in_flight_fence);
    ...
    vkResetCommandBuffer(ren->command_buffer, 0);
    vk::record_command_buffer(ren, ren->command_buffer, image_idx);

    VkSubmitInfo submit_info{};
    submit_info.sType = VK_STRUCTURE_TYPE_SUBMIT_INFO;

    VkSemaphore wait_semaphores[] = { ren->image_available_semaphore };
    VkPipelineStageFlags wait_stages[] = { VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT };
    submit_info.waitSemaphoreCount = 1;
    submit_info.pWaitSemaphores = wait_semaphores;
    submit_info.pWaitDstStageMask = wait_stages;
    submit_info.commandBufferCount = 1;
    submit_info.pCommandBuffers = &ren->command_buffer;

    VkSemaphore signal_semaphores[] = { ren->render_finished_semaphore };
    submit_info.signalSemaphoreCount = 1;
    submit_info.pSignalSemaphores = signal_semaphores;

    if (vkQueueSubmit(ren->graphics_queue, 1, &submit_info, ren->in_flight_fence) != VK_SUCCESS) {
...
extern "C" void ren_destroy(t_ren* ren) {
    ...
    vkDestroySemaphore(ren->device, ren->image_available_semaphore, nullptr);
    vkDestroySemaphore(ren->device, ren->render_finished_semaphore, nullptr);
    vkDestroyFence(ren->device, ren->in_flight_fence, nullptr);
